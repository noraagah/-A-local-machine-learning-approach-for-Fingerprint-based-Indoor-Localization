%File Name: RSSI_Prediction_Vectorized
...
%latitude, and floor
%First created: 3/19/22
%Last Modified: 3/19/22
...
function [RSSI_Predictions] = RSSI_Prediction_Vectorized(RSSI_Data, Location_Matrix)
    ...
    ha = (1/6) * 1.06 * std(RSSI_Data.location_train_df(:, 313)) * (N/3)^(-1/6);
    ho = (1/6) * 1.06 * std(RSSI_Data.location_train_df(:, 314)) * (N/3)^(-1/6);
    ...
    RSSI_Predictions_byproduct1 = ones(Num_Locs, Num_RSSIs);
    RSSI_Predictions_byproduct2 = ones(Num_Locs, Num_RSSIs);
    ...
    %for k = 1:Num_RSSIs
        ...
        for j = 1:Num_Locs
            ...
            %for i = 1:N
                ...
                na = normpdf((RSSI_Data.location_train_df(:, 313)-Location_Matrix(j, 1))/ha);
                no = normpdf((RSSI_Data.location_train_df(:, 314)-Location_Matrix(j, 2))/ho);
        
                %Indicator function for the floor, if the floor is the same count
                %the data point
                Indic = (RSSI_Data.location_train_df(:, 315) == Location_Matrix(j, 3));
        
                %Multiply the appropriate values, then sum for all of the
                %samples, divide 
                top = sum((RSSI_Data.location_train_df(:, 1:312)).*(na.*no.*Indic), 1)/(N*ha*ho);
                bottom = sum(na.*no.*Indic, 1)/(N*ha*ho);
            %end
       
            RSSI_Predictions(j, :) = ((top+ee*(-125))./bottom+ee);
%             RSSI_Predictions_byproduct1(j, :) = top;
%             RSSI_Predictions_byproduct2(j, :) = bottom;
%     
        end
%         RSSI_Predictions_byproduct1(j,:)
%         RSSI_Predictions_byproduct2(j,:)
    %end
