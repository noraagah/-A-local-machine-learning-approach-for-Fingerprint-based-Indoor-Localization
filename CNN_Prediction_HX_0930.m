...
% Load in floor training data
    ...
    %A = arrayDatastore(Training_Image_Folder);
    for i = 1:length(imageFiles)/5
        j=5*(i-1)+1;
        arrds = load(fullfile(imageFiles(j).folder,imageFiles(j).name));
...
%     for i = 1:10
%        floor_test_data(:,:,1,i+1) -       floor_test_data(:,:,1,i) 
%     end
...
%     flip(responses, 1);
    ...
    responses = abs(Loc_Train_Observations(:,314))/1000;
...
%                 reluLayer
    ...
    %histogram(Floor_in_sample_prediction)
% 
%     aa=2022;
%     %hold on
%     %histogram(Floor_Train_Observations(:,315))
%     hold on
%     histogram((Floor_in_sample_prediction-Floor_Train_Observations(:,315)))
    


%     size(M_in_sample)
%     size(Floor_Prediction_in_sample)
% 
%     M_in_sample(1:100)
% 
%     Floor_Prediction_in_sample(1:2000)
% 
%     std(Floor_Prediction_in_sample)
%     histogram(responses)

    %histogram(Floor_Prediction_in_sample)



    %errorcheck=(Floor_prediction_in_sample == responses);
    %size(errorcheck)
...
%    Floor_prediction_out_sample= predict(convnet_Floor, floor_test_data);

%     for k=1:10
%     %floor_test_data(:,:,1,k+1)-floor_test_data(:,:,1,k)
%     floor_test_data(:,:,1,k)
%     end
    %Floor_prediction_in_sample(1:100,:)
...
% 
%     for ii = 1:10
%         arrds = load(fullfile(imageFiles(ii).folder,imageFiles(ii).name));
%         arrds.RSSI_Image_Matrix;
%     end
