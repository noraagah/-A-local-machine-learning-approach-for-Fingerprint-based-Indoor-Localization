...
Floor_train = Data1.location_train_df;
...
[CNN_Longitude_Prediction] = CNN_Longitude_Prediction_1209(Floor_train, Floor_test)

diff=abs(Floor_test(:,size(Floor_test,2)-3:size(Floor_test,2)-2))-abs(CNN_Longitude_Prediction);
diff_sq = diff.*diff;
error=sqrt((diff_sq(:,1)+diff_sq(:,2)));
mean(error)
